...
(function() {
     ...
     */
    function fixBaselineError() {
        log("Aplicando correção para o erro do baseline...");
        
        // Guardar referência original para executeSelectedPlaybooks
        ...
        window.executeSelectedPlaybooks = function() {
            try {
                // Verificar se uma playbook baseline está selecionada
                ...
                for (const playbook of selectedPlaybooks) {
                    ...
                    if (playbookName && playbookName.toLowerCase().includes('baseline')) {
                ...
                if (baselineSelected) {
                    log("Playbook baseline detectada, aplicando validação adicional...");
                    
                    // Verificação específica para validar antes de executar
                    // Aviso ao usuário sobre possível erro
                    const continuar = confirm(
                        "ATENÇÃO: A playbook baseline pode conter um erro de sintaxe.\n\n" +
                        "Possível erro na linha 2: 'debug' não é um atributo válido para um Play.\n\n" +
                        "É recomendado que você verifique e corrija o arquivo antes de executar.\n\n" +
                        "Deseja tentar executar mesmo assim?"
                    );
                    
                    if (!continuar) {
                        log("Execução cancelada pelo usuário após aviso sobre erro do baseline");
                        showUserMessage("Execução cancelada. Verifique e corrija o arquivo baseline_universal.yml", "warning");
                        return;
                    }
                    
                    // Se o usuário optar por continuar, forneça instruções de correção
                    showUserMessage(
                        "Prosseguindo com a execução, mas erros podem ocorrer. Se ocorrerem, edite o arquivo baseline_universal.yml e remova ou corrija a propriedade 'debug' na linha 2.",
                        "warning",
                        8000
                    );
                }
                ...
                log(`Erro ao validar baseline: ${error.message}`, 'error');
                return originalExecuteSelectedPlaybooks.apply(this, arguments);
            }
        };
        
        log("Correção do erro do baseline aplicada com sucesso!");
    }
