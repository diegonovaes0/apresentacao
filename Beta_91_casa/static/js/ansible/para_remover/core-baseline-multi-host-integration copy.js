...
// Melhorar a função injectLog para lidar com múltiplos hosts
...
// Atualizar tarefas no log - com filtro específico para o host
function updateTasks(jobId, output) {
    // Verificar se é um jobId composto (com hostname)
    ...
    // Se é específico para um host, filtramos a saída apenas para esse host
    if (targetHostname) {
        const execution = state.activeExecutions.get(jobId);
        if (!execution) return;
        
        // Filtrar a saída para mostrar apenas as linhas relevantes para este host
        const filteredOutput = output.split('\n')
            .filter(line => {
                // Incluir linhas gerais e linhas específicas para este host
                return !line.includes("[") || 
                       line.includes(`[${targetHostname}]`) || 
                       line.toLowerCase().includes(targetHostname.toLowerCase());
            })
            .join('\n');
        
        // Usar a saída filtrada para este host específico
        processHostOutput(execution, filteredOutput);
    } else {
        // Caso não seja um jobId composto, processa normalmente
        const execution = state.activeExecutions.get(jobId);
        if (!execution) return;
        processHostOutput(execution, output);
    }
}
...
// Função auxiliar para processar a saída para um host específico
function processHostOutput(execution, output) {
...
function interceptPlaybookExecution() {
    ...
    window.executeSelectedPlaybooks = function() {
        ...
        if (hasBaseline) {
            ...
            // Modificação: Criar um único payload para todos os hosts
            ...
            window.fetch = function(url, options) {
                if (url === '/api/run' && options?.method === 'POST') {
                    ...
                    if (isBaseline && data.hosts && data.hosts.length > 0) {
                        // Preparar variáveis extras consolidadas que funcionam para todos os hosts
                        const extra_vars = {
                            multi_host_execution: true,
                            hosts_config: {}
                        };
                        
                        // Coletar configurações para cada host
                        data.hosts.forEach(hostname => {
                            ...
                            // Armazenar configurações para este host
                            extra_vars.hosts_config[hostname] = {
                        ...
                        // Definir as variáveis extras no payload
                        data.extra_vars = extra_vars;
                        
                        // Substituir o corpo da requisição
                        options.body = JSON.stringify(data);
                        
                        console.log("Enviando execução de baseline com múltiplos hosts:", data);
                    }
                }
...
// Modificar a função que cria cards para suportar múltiplos hosts no mesmo card
function interceptExecutionCards() {
    ...
    window.createExecutionCard = function(playbookName, hosts, jobId) {
        ...
        if (isBaselinePlaybook(playbookName)) {
            // Para playbooks baseline, criar um log para cada host no mesmo card
            setTimeout(() => {
                const hostsArray = Array.from(hosts);
                hostsArray.forEach(hostname => {
                    injectLog(card, jobId, hostname);
                });
                
                // Adicionar um indicador de múltiplos hosts se necessário
                if (hostsArray.length > 1) {
                    const hostInfo = card.querySelector('.host-info');
                    if (hostInfo) {
                        const multiHostBadge = document.createElement('div');
                        multiHostBadge.className = 'multi-host-badge';
                        multiHostBadge.textContent = `${hostsArray.length} hosts`;
                        multiHostBadge.style.cssText = `
                            ...
                            display: inline-block;
                        `;
                        hostInfo.insertBefore(multiHostBadge, hostInfo.firstChild);
                    }
                }
            }, 500);
        }
...
// Interceptar monitoramento de execuções para suportar vários hosts
function interceptExecutionMonitoring() {
    ...
    window.monitorPlaybookExecution = function(jobId, card) {
        ...
        // Para playbooks baseline, buscar status para cada host no card
        ...
        if (hostDetails.length > 0) {
            hostDetails.forEach(hostDetail => {
                ...
                // Configurar intervalo específico para cada host
                const hostJobId = jobId + '-' + hostname;
                ...
                const interval = setInterval(() => {
                    fetch(`/api/status/${jobId}`)
                        .then(res => res.json())
                        .then(data => {
                            // Atualizar logs específicos do host
                            updateTasks(hostJobId, data.output || '');
                            updateStatus(hostJobId, data.status);
                        ...
                        .catch(err => {
                            console.error(`Erro ao atualizar log para ${hostname}:`, err);
                            clearInterval(interval);
                        });
                }, 1000);
            });
