...
SIXTY_FOUR_BIT_ARM_INSTALL_FILE='arm/Site24x7_Linux_64bit.install'
	...
	if [ "$LOCAL_SETUP" = "$BOOL_TRUE" ]; then
	   if [ "$IS_VENV_SUPPORT_NEEDED" = "$BOOL_FALSE" ]; then
		if [ "$OS_BINARY_TYPE" = "$THIRTY_TWO_BIT" ]; then
			...
			DOWNLOAD_URL="$LOCAL_SERVER/$SIXTY_FOUR_BIT_ARM_INSTALL_FILE"
		else
		...
		if [ "$OS_BINARY_TYPE" = "$THIRTY_TWO_BIT" ]; then
			...
			DOWNLOAD_URL="$SERVER/server/$SIXTY_FOUR_BIT_ARM_INSTALL_FILE"
		else
