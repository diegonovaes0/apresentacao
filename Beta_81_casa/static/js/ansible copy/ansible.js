...
/**
 * Cria o HTML do banner de host com layout aninhado (quadrados e retângulo)
 * @param {string} hostname - Nome do host
 * @param {Object} info - Informações do host
 * @returns {string} HTML do banner
 */
/* Função updateHostBanner permanece igual */
function updateHostBanner(hostname, info) {
    ...
    if (info.valid) {
        ...
        const systemDisplay = facts.system || 'Sistema Operacional';
        
        return `
            <div class="host-banner ${validClass} ${selectedClass}">
                <div class="host-header">
                    <h4>${displayHostname}</h4>
                </div>
                <div class="host-content">
                    <div class="host-info-horizontal">
                        ...
                        <span>
                            ...
                            <strong>Sistema:</strong> ${systemDisplay}
                        </span>
                    </div>
                </div>
                ...
                <div class="host-content">
                    <div class="host-info-horizontal">
                        <p>
                            ...
                            <strong>Status:</strong> Host inacessível ou desligado
                        </p>
                    </div>
                </div>
                <div class="host-footer">
...
async function cancelExecution(button) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Erro ao cancelar job ${jobId}`);
        handlePlaybookCompletion('cancelled', card);
        runningJobs.delete(jobId);
        debugLog(`Job ${jobId} cancelado com sucesso`);
    } catch (error) {
        debugLog(`Erro ao cancelar job ${jobId}: ${error.message}`, 'error');
    }
}
...
async function cancelAllExecutions() {
    try {
        ...
        alert(`Erro ao cancelar execuções: ${error.message}`);
    }
}
...
function enhanceHostBanners() {
    document.querySelectorAll('.host-banner').forEach(banner => {
        ...
        // Adiciona badge para o sistema operacional
        const systemText = banner.querySelector('p:nth-of-type(3)').textContent.replace('Sistema: ', '');
        if (systemText && systemText !== 'N/A') {
            const osBadge = document.createElement('div');
            osBadge.className = 'os-badge';
            
            if (systemText.toLowerCase().includes('linux') || systemText.toLowerCase().includes('ubuntu') || systemText.toLowerCase().includes('debian') || systemText.toLowerCase().includes('centos') || systemText.toLowerCase().includes('fedora')) {
                osBadge.innerHTML = `
