...
function interceptPlaybookExecution() {
    ...
    window.executeSelectedPlaybooks = function() {
        ...
        if (hasBaseline) {
            ...
            // Interceptação para definir variáveis extras por host
            ...
            window.fetch = function(url, options) {
                if (url === '/api/run' && options?.method === 'POST') {
                    ...
                    if (isBaseline && data.hosts) {
                        // Para múltiplos hosts, processar um por um
                        if (data.hosts.length > 1) {
                            console.log(`Processando baseline para múltiplos hosts: ${data.hosts.join(', ')}`);
                            
                            // Vamos executar cada host individualmente, mas em sequência
                            const hosts = [...data.hosts];
                            
                            // Executar o primeiro host
                            executeHostsSequentially(hosts, data.playbook);
                            
                            // Impedir a execução do fetch original
                            return new Promise(() => {});
                        }
                        // Para um único host, continuar com a lógica original
...
// Função para executar hosts em sequência
function executeHostsSequentially(hosts, playbookPath) {
    if (hosts.length === 0) {
        console.log("Todos os hosts foram processados");
        return;
    }
    
    const hostname = hosts.shift(); // Remove o primeiro host da lista
    ...
    if (!parceiroPassword || parceiroPassword.length < 8 || !rootPassword || rootPassword.length < 8) {
        ...
        // Continuar com o próximo host mesmo se este falhar
        executeHostsSequentially(hosts, playbookPath);
        return;
    }
    ...
    .then(result => {
        console.log(`Baseline iniciado para host ${hostname}: job ${result.job_id}`);
        
        // Adicionar pequeno atraso antes de executar o próximo host
        setTimeout(() => {
            executeHostsSequentially(hosts, playbookPath);
        }, 1000);
    })
    .catch(error => {
        console.error(`Erro ao executar baseline para host ${hostname}: ${error.message}`);
