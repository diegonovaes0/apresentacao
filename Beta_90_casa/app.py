...
"""
    ...
    # Atualize a função cancel_playbook na classe AnsibleManager
def cancel_playbook(self, job_id: str) -> bool:
    ...
    try:
        ...
        # Marcando o status como cancelado
        self.running_playbooks[job_id]["status"] = "cancelled"
        self.running_playbooks[job_id]["progress"] = 100
        self.running_playbooks[job_id]["output"] += "\n\n---- Cancelado pelo usuário ----"
        
        # Encontrar o processo associado e terminar
        if hasattr(subprocess.Popen, 'running_processes'):
            for process in list(subprocess.Popen.running_processes):  # Criar uma cópia da lista para evitar problemas
                if hasattr(process, 'job_id') and process.job_id == job_id:
                    try:
                        # Enviar sinal para interromper o processo
                        process.terminate()
                        try:
                            # Aguardar o término com timeout
                            process.wait(timeout=2)
                        except subprocess.TimeoutExpired:
                            # Se não terminar no tempo, força o término
                            process.kill()
                            
                        logger.info(f"Processo do job {job_id} terminado")
                        subprocess.Popen.running_processes.remove(process)
                    except Exception as proc_error:
                        logger.error(f"Erro ao terminar processo do job {job_id}: {str(proc_error)}")
...
def cancel_job(job_id):
    """
    Endpoint para cancelar um job específico pelo ID.
    O Flask espera a rota neste formato, não como POST JSON para /api/cancel.
    """
    try:
        logger.info(f"Requisição para cancelar job: {job_id}")
        
        if not job_id:
            logger.error("Job ID não fornecido")
        ...
        # Encontrar e matar o processo associado
        if hasattr(subprocess.Popen, 'running_processes'):
            for process in subprocess.Popen.running_processes:
                if hasattr(process, 'job_id') and process.job_id == job_id:
                    try:
                        ...
                        logger.warning(f"Não foi possível terminar o processo do job {job_id}")
        ...
        if success:
            ...
            logger.error(f"Falha ao cancelar job {job_id}")
            return jsonify({"success": False, "message": f"Falha ao cancelar job {job_id}"}), 500
