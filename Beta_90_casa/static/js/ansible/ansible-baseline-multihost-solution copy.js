...
// Substituir a função saveHostConfiguration
function saveHostConfiguration(hostname, banner) {
    ...
    // Atualizar badges imediatamente após salvar
    setTimeout(() => {
        refreshBaselineBadges();
    }, 100);
    
    // Verificar se todos os hosts estão configurados
    setTimeout(() => {
...
// Função para inicializar as correções
function initializeBaselineFixes() {
    console.log("[Ansible Multi-Host] Inicializando correções de baseline");
    ...
    // Iniciar verificações periódicas de badges
    setInterval(refreshBaselineBadges, 2000);
    
    console.log("[Ansible Multi-Host] Correções de baseline inicializadas com sucesso");
}

// Inicializar as correções quando o documento estiver pronto
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeBaselineFixes);
} else {
    // Se o DOM já estiver carregado, inicializar imediatamente
    initializeBaselineFixes();
}
    /**
   ...
   // Substituir a função refreshBaselineBadges
function refreshBaselineBadges() {
    ...
    console.log("[Ansible Multi-Host] Adicionando emblemas de baseline com cores corretas");
    ...
    selectedHosts.forEach(hostElement => {
        ...
        // Verificar se o host está corretamente configurado
...
if (document.readyState === 'loading') {
   ...
   // Substituir a função validateHostsConfiguration
function validateHostsConfiguration() {
    ...
    if (unconfiguredHosts.length > 0) {
        ...
        // Exibir mensagem personalizada para um ou múltiplos hosts
        const message = unconfiguredHosts.length === 1 
            ? `Configure o baseline para o host ${unconfiguredHosts[0]} antes de executar esta playbook`
            : `Configure o baseline para os hosts antes de executar esta playbook`;
        
        showMessage(message, 'warning', 5000);
        
        // Garantir que os badges estejam atualizados
        refreshBaselineBadges();
...
// Função auxiliar para obter a lista de hosts não configurados
...
// Corrigir o estilo dos badges de baseline
function fixBaselineBadges() {
    const style = document.createElement('style');
    style.id = 'baseline-badge-fix';
    style.textContent = `
        /* Estilo para badge de baseline quando não configurado (vermelho) */
        .baseline-badge.required {
        ...
        /* Estilo para badge de baseline quando configurado (verde) */
        .baseline-badge:not(.required) {
            background-color: #4CAF50 !important;
            color: white !important;
        }
        
        /* Garante visibilidade dos badges */
        .baseline-badge {
            opacity: 1 !important;
            z-index: 100 !important;
        }
        
        /* Animação de pulso mais visível */
        @keyframes pulse {
            0% { transform: scale(1); }
...
/**
 * Correção do problema de validação do Baseline Multihost
 * 
 * Este script corrige o comportamento da validação de baseline,
 * garantindo que:
 * 1. Cards não sejam criados para hosts não configurados
 * 2. Badges de baseline apareçam corretamente (vermelho/verde)
 * 3. Mensagens de erro sejam exibidas quando necessário
 */

// Substituir completamente a função interceptPlaybookExecution
function interceptPlaybookExecution() {
        ...
        window.executeSelectedPlaybooks = function() {
            ...
            if (isAnyBaselineSelected()) {
                ...
                if (!validateHostsConfiguration()) {
                    ...
                    // Abrir banners de configuração para hosts não configurados
                    const unconfiguredHosts = getUnconfiguredHosts();
                    if (unconfiguredHosts.length > 0) {
                        // Abrir o banner do primeiro host não configurado
                        if (!STATE.hostBanners.has(unconfiguredHosts[0])) {
                            addConfigBanner(unconfiguredHosts[0]);
                        }
                    }
            ...
            // Prosseguir com a execução normal se a validação passou
    ...
    // Substituir a função createExecutionCard para bloquear a criação do card
        ...
        window.createExecutionCard = function(playbookName, hosts, jobId) {
            ...
            if (isBaselinePlaybook(playbookName)) {
                ...
                if (unconfiguredHosts.length > 0) {
                    ...
                    const message = unconfiguredHosts.length === 1 
                        ? `Configure o baseline para o host ${unconfiguredHosts[0]} antes de executar esta playbook`
                        : `Configure o baseline para todos os hosts antes de executar esta playbook`;
                    ...
                    // Retornar um elemento vazio para não quebrar o fluxo
            ...
            // Criar o card normalmente se passar na validação
