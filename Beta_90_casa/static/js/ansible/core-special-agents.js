...
function updateButtonLabels() {
    createdButtons.forEach((btn, playbookName) => {
        ...
        // Verificar se está configurado e verificado - AGORA VERIFICA ESPECIFICAMENTE POR TIPO
        let isVerified = false;
        
        if (isSite24x7 && window.configState.site24x7 && window.configState.site24x7.verified) {
            isVerified = true;
        } else if (isAntivirus && window.configState.antivirus && window.configState.antivirus.verified) {
            isVerified = true;
        }
...
function createAndPositionButtons() {
    ...
    document.querySelectorAll('.playbook-item').forEach(item => {
        ...
        // Determinar status de verificação - AGORA ESPECÍFICO POR TIPO DE PLAYBOOK
        let isVerified = false;
        ...
        btn.setAttribute('data-type', isSite24x7 ? 'site24x7' : 'antivirus');
